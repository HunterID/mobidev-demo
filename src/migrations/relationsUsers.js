...
async function randomRelations() {
    ...
    while (createdRelations < MAX_AMOUNT_NEW_RELATIONS) {
        const [firstRandomNumber, secondRandomNumber] = generateTwoRandomNumbers(MAX_AMOUNT_NEW_USERS);
        ...
        const isBothRandomUsersAreFriends =
            firstRandomUser.friends.some((id) => secondRandomUserId.toString() === id.toString()) &&
            secondRandomUser.friends.some((id) => firstRandomUserId.toString() === id.toString());

        if (isBothRandomUsersAreFriends) {
            continue;
        }

        const isFirstRandomUserFollowedBySecond = firstRandomUser.followers.some((id) => secondRandomUserId.toString() === id.toString());
        const isSecondRandomUserFollowedByFirst = secondRandomUser.followers.some((id) => firstRandomUserId.toString() === id.toString());

        if (isFirstRandomUserFollowedBySecond && isSecondRandomUserFollowedByFirst) {
            await Promise.all([
                Users.updateOne(
        ...
        await Users.updateOne({ _id: firstRandomUser._id }, { $push: { followers: Types.ObjectId(secondRandomUserId) } });
        ++createdRelations;
        continue;
    }
}
